...
const getFielName = (str) => {
  const temp = str.split('assignments/');
  const arr = temp[temp.length - 1].split('.');
    ...
    selected.map((select => {
      ...
      axios.get(link).then((res) => {
        ...
        datas.map((data) => {
          ...
          let ext = getFileExtension(data.file)
          if (ext !== 'docx' && ext !== 'pdf' && ext !== 'hwp' && ext !== 'jpg' && ext !== 'jpeg' && ext !== 'png' && ext !== 'DOCX' && ext !== 'PDF' && ext !== 'HWP' && ext !== 'JPG' && ext !== 'JPEG' && ext !== 'PNG' && ext !== 'zip' && ext !== 'ZIP') ext = 'docx'
          let filename = name + `.${ext}`;
          console.log(data.file)
          JSZipUtils.getBinaryContent(data.file, async function (err, data) {
            if (err) {
              throw err;
            }
